...
class NewsSpider(scrapy.Spider):
    ...
    def init_attributes(
        ...
        self.start_pageid = start_pageid
        self.end_pageid = end_pageid
        self.browsing = browsing
        if browsing == "date":
            self.num_pages = (end_date - start_date).days
        elif browsing == "pageid":
            self.num_pages = end_pageid - start_pageid + 1
    ...
    def get_page_urls(self, base_url):
        page_urls = []
        if self.browsing == "date":
            current_date = self.start_date
            for i in range(0, self.num_pages + 1):
                page_urls.append(base_url + current_date.strftime('%Y-%m-%d') + "/")
                current_date += datetime.timedelta(days=1)
        elif self.browsing == "pageid":
            for i in range(self.start_pageid, self.end_pageid + 1):
                page_urls.append(base_url.format(i))

        return page_urls

    def parse_page(self, response):
        links = response.xpath(self.link_pattern)
        if len(links) > 0:
            for href in links:
                url = self.start_urls[0] + "/" + href.extract()
                if url.find('javascript') == -1:
                    self.num_requested_articles += 1
                    print("Requested articles: %d" % self.num_requested_articles)
